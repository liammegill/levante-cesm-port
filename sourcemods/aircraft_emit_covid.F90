...
!-----------------------------------------------------------------------
  ...
  integer, public, parameter :: N_AERO = 13
  character(len=13), public    :: aero_names(N_AERO) = (/'ac_SLANT_DIST','ac_TRACK_DIST',           &
                          'ac_HC      ','ac_NOX     ','ac_PMNV    ',                            &
...
!         forcings_air(m)%file%curr_filename    = spc_fname(m)
    ...
    do m=1,aircraft_cnt
       ...
       ! TODO: data_cycle_yr is hardcoded to 2019 below (was 0 in Gettelman's original
       ! code, which mismatched our RUN_REFDATE/RUN_STARTDATE combination). Should be a
       ! namelist-configurable aircraft_cycle_yr variable instead, following the same
       ! pattern as aircraft_datapath/aircraft_specifier in aircraft_emit_readnl below,
       ! so this doesn't need a source edit for every future simulation year.
       call trcdata_init( forcings_air(m)%sectors, forcings_air(m)%filename, forcings_air(m)%filelist, air_datapath, &
                          forcings_air(m)%fields, forcings_air(m)%file, rmv_file, 2019, 0, 0, air_type)
