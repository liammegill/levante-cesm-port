...
!    use phys_buffer,    only: pbuf_size_max, pbuf_fld, pbuf_old_tim_idx, pbuf_get_fld_idx, pbuf_times
    ...
    real(r8), parameter :: radius = 3.75e-6            ! radius of ice particle = 7.5 microns
...
!--------------------------------------------------------------------------------------------
! consider ambient ice supersaturation, volume = slant_dist x 100 m x 100 m
!--------------------------------------------------------------------------------------------
...
!    ifld = pbuf_get_fld_idx('ac_H2O')
!    ac_H2O => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk,itim)
!    ifld = pbuf_get_fld_idx('ac_SLANT_DIST')
!    ac_SLANT_DIST => pbuf(ifld)%fld_ptr(1,1:pcols,1:pver,lchnk,itim)
