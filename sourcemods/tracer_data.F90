...
!    select case ( to_upper(data_type) )
    ...
    ! Fix: original check only exempted 'CYCLICAL' type, but the code below
    ! ("if (file%cyclical .or. file%cyclical_list) ... if (file%cyc_yr /= 0)")
    ! clearly uses cyc_yr for CYCLICAL_LIST too -- the validation was incomplete.
